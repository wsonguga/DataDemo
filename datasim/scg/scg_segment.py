...
def scg_segment(scg_cleaned, rpeaks=None, sampling_rate=1000, show=False):
    """Segment an scg signal into single heartbeats.
    ...
    # Sanitize inputs
    if rpeaks is None:
        _, rpeaks = scg_peaks(scg_cleaned, sampling_rate=sampling_rate, correct_artifacts=True)
        rpeaks = rpeaks["scg_R_Peaks"]

    epochs_start, epochs_end = _scg_segment_window(
        rpeaks=rpeaks, sampling_rate=sampling_rate, desired_length=len(scg_cleaned)
    )
    heartbeats = epochs_create(
        scg_cleaned, rpeaks, sampling_rate=sampling_rate, epochs_start=epochs_start, epochs_end=epochs_end
    )
...
def _scg_segment_window(heart_rate=None, rpeaks=None, sampling_rate=1000, desired_length=None):
