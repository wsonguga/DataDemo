...
def scg_analyze(data, sampling_rate=1000, method="auto"):
    """Performs scg analysis on either epochs (event-related analysis) or on longer periods of data such as resting-
    state data.
    ...
    data : Union[dict, pd.DataFrame]
        ...
        DataFrame of processed signals from a longer period of data, typically generated by `scg_process()`
    ...
    -------
    DataFrame
        A dataframe containing the analyzed scg features. If
        event-related analysis is conducted, each epoch is indicated
        by the `Label` column. See `scg_eventrelated()` and
        `scg_intervalrelated()` docstrings for details.

    See Also
    --------
    bio_process, scg_process, epochs_create, scg_eventrelated, scg_intervalrelated
    ...
    >>>
    >>> # Analyze
    >>> analyze_epochs = nk.scg_analyze(epochs, sampling_rate=100)
    ...
    >>>
    >>> # Analyze
    >>> analyze_df = nk.scg_analyze(df, sampling_rate=100)
    ...
    if method in ["event-related", "event", "epoch"]:
        ...
        if len([i for i in colnames if "Label" in i]) == 0:
            raise ValueError(
                "NeuroKit error: scg_analyze(): Wrong input or method,"
                "we couldn't extract epochs features."
            )
        else:
            features = scg_eventrelated(data)

    # Interval-related analysis
    elif method in ["interval-related", "interval", "resting-state"]:
        features = scg_intervalrelated(data, sampling_rate=sampling_rate)
