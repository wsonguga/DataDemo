...
def scg_findpeaks(scg_cleaned, sampling_rate=1000, method="neurokit", show=False):
    ...
    if isinstance(scg_cleaned, pd.DataFrame):
        try:
            scg_cleaned = scg_cleaned["scg_Clean"]
        except NameError:
            try:
                scg_cleaned = scg_cleaned["scg_Raw"]
            except NameError:
                scg_cleaned = scg_cleaned["scg"]

    # Sanitize input
    scg_cleaned = signal_sanitize(scg_cleaned)

    method = method.lower()  # remove capitalised letters
    # Run peak detection algorithm
    if method in ["nk", "nk2", "neurokit", "neurokit2"]:
        rpeaks = _scg_findpeaks_neurokit(scg_cleaned, sampling_rate, show=show)
    elif method in ["pantompkins", "pantompkins1985"]:
        rpeaks = _scg_findpeaks_pantompkins(scg_cleaned, sampling_rate)
    elif method in ["nabian", "nabian2018"]:
        rpeaks = _scg_findpeaks_nabian2018(scg_cleaned, sampling_rate)
    elif method in ["gamboa2008", "gamboa"]:
        rpeaks = _scg_findpeaks_gamboa(scg_cleaned, sampling_rate)
    elif method in ["ssf", "slopesumfunction", "zong", "zong2003"]:
        rpeaks = _scg_findpeaks_ssf(scg_cleaned, sampling_rate)
    elif method in ["hamilton", "hamilton2002"]:
        rpeaks = _scg_findpeaks_hamilton(scg_cleaned, sampling_rate)
    elif method in ["christov", "christov2004"]:
        rpeaks = _scg_findpeaks_christov(scg_cleaned, sampling_rate)
    elif method in ["engzee", "engzee2012", "engzeemod", "engzeemod2012"]:
        rpeaks = _scg_findpeaks_engzee(scg_cleaned, sampling_rate)
    elif method in ["elgendi", "elgendi2010"]:
        rpeaks = _scg_findpeaks_elgendi(scg_cleaned, sampling_rate)
    elif method in ["kalidas2017", "swt", "kalidas", "kalidastamil", "kalidastamil2017"]:
        rpeaks = _scg_findpeaks_kalidas(scg_cleaned, sampling_rate)
    elif method in ["martinez2003", "martinez"]:
        rpeaks = _scg_findpeaks_WT(scg_cleaned, sampling_rate)
    elif method in ["rodrigues2020", "rodrigues2021", "rodrigues", "asi"]:
        rpeaks = _scg_findpeaks_rodrigues(scg_cleaned, sampling_rate)
    elif method in ["promac", "all"]:
        rpeaks = _scg_findpeaks_promac(scg_cleaned, sampling_rate=sampling_rate, threshold=0.33, show=show)
    else:
        raise ValueError("NeuroKit error: scg_findpeaks(): 'method' should be one of 'neurokit'" "or 'pamtompkins'.")

    # Prepare output.
    info = {"scg_R_Peaks": rpeaks}
...
# =============================================================================
def _scg_findpeaks_promac(signal, sampling_rate=1000, threshold=0.33, show=False, **kwargs):

    x = np.zeros(len(signal))

    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_neurokit, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_gamboa, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_ssf, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_engzee, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_elgendi, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_kalidas, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_WT, **kwargs)
    x = _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, _scg_findpeaks_rodrigues, **kwargs)
...
def _scg_findpeaks_promac_addmethod(signal, sampling_rate, x, fun, **kwargs):
    peaks = fun(signal, sampling_rate=sampling_rate, **kwargs)
    x += _scg_findpeaks_promac_convolve(signal, peaks, sampling_rate=sampling_rate)
    return x


def _scg_findpeaks_promac_convolve(signal, peaks, sampling_rate=1000):
    x = np.zeros(len(signal))
    x[peaks] = 1
...
def _scg_findpeaks_neurokit(
    ...
    # Compute the scg's gradient as well as the gradient threshold. Run with
...
# Pan & Tompkins (1985)
# =============================================================================
def _scg_findpeaks_pantompkins(signal, sampling_rate=1000):
    ...
    mwa = _scg_findpeaks_MWA(squared, N)
    mwa[: int(0.2 * sampling_rate)] = 0

    mwa_peaks = _scg_findpeaks_peakdetect(mwa, sampling_rate)
...
# Nabian et al. (2018)
# ===========================================================================
def _scg_findpeaks_nabian2018(signal, sampling_rate=1000):
    ...
    for i in range(1+window_size, len(signal)-window_size):
        scg_window = signal[i-window_size:i+window_size]
...
# Hamilton (2002)
# =============================================================================
def _scg_findpeaks_hamilton(signal, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/
...
# =============================================================================
def _scg_findpeaks_ssf(signal, sampling_rate=1000, threshold=20, before=0.03, after=0.01):
...
# Christov (2004)
# =============================================================================
def _scg_findpeaks_christov(signal, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/
...
# Gamboa (2008)
# =============================================================================
def _scg_findpeaks_gamboa(signal, sampling_rate=1000, tol=0.002):
    """From https://github.com/PIA-
...
# =============================================================================
def _scg_findpeaks_engzee(signal, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/
...
# =============================================================================
def _scg_findpeaks_kalidas(signal, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/
    ...
    try:
        ...
        raise ImportError(
            "NeuroKit error: scg_findpeaks(): the 'PyWavelets' module is required for"
    ...
    swt_scg = pywt.swt(signal, "db3", level=swt_level)
    swt_scg = np.array(swt_scg)
    swt_scg = swt_scg[0, 1, :]

    squared = swt_scg * swt_scg
    ...
    filt_peaks = _scg_findpeaks_peakdetect(filtered_squared, sampling_rate)
...
# Elgendi et al. (2010)
# =============================================================================
def _scg_findpeaks_elgendi(signal, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/
    ...
    mwa_qrs = _scg_findpeaks_MWA(abs(signal), window1)

    window2 = int(0.6 * sampling_rate)
    mwa_beat = _scg_findpeaks_MWA(abs(signal), window2)
...
# =============================================================================
#
def _scg_findpeaks_WT(signal, sampling_rate=1000):
    # Try loading pywt
    try:
        import pywt
    except ImportError:
        raise ImportError(
            "NeuroKit error: scg_delineator(): the 'PyWavelets' module is required for"
            " this method to run. Please install it first (`pip install PyWavelets`)."
        )
...
def _scg_findpeaks_rodrigues(signal, sampling_rate=1000):
    ...
    diff_scg = [signal[i] - signal[i - Nd] for i in range(Nd, len(signal))]
    ddiff_scg = [diff_scg[i] - diff_scg[i - 1] for i in range(1, len(diff_scg))]
    squar = np.square(ddiff_scg)

    # Integrate moving window
    b = np.array(np.ones(N))
    a = [1]
    processed_scg = scipy.signal.lfilter(b, a, squar)
    ...
    while i < tf - sampling_rate:  # ignore last second of recording
        ...
        while i < tf1:
            ...
            if processed_scg[i] > Rpeakamp:
                Rpeakamp = processed_scg[i]
                rpeakpos = i + 1
            i += 1
...
def _scg_findpeaks_MWA(signal, window_size):
    """From https://github.com/berndporr/py-scg-detectors/"""
...
def _scg_findpeaks_peakdetect(detection, sampling_rate=1000):
    """From https://github.com/berndporr/py-scg-detectors/"""
