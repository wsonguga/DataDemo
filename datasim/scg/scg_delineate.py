...
def scg_delineate(
    scg_cleaned, rpeaks=None, sampling_rate=1000, method="peak", show=False, show_type="peaks", check=False
):
    ...
    """
    # Sanitize input for scg_cleaned
    if isinstance(scg_cleaned, pd.DataFrame):
        cols = [col for col in scg_cleaned.columns if "scg_Clean" in col]
        if cols:
            scg_cleaned = scg_cleaned[cols[0]].values
        else:
            raise ValueError("NeuroKit error: scg_delineate(): Wrong input, we couldn't extract" "cleaned signal.")

    elif isinstance(scg_cleaned, dict):
        for i in scg_cleaned:
            cols = [col for col in scg_cleaned[i].columns if "scg_Clean" in col]
            if cols:
                signals = epochs_to_df(scg_cleaned)
                scg_cleaned = signals[cols[0]].values
    ...
    if method in ["peak", "peaks", "derivative", "gradient"]:
        waves = _scg_delineator_peak(scg_cleaned, rpeaks=rpeaks, sampling_rate=sampling_rate)
    elif method in ["cwt", "continuous wavelet transform"]:
        waves = _scg_delineator_cwt(scg_cleaned, rpeaks=rpeaks, sampling_rate=sampling_rate)
    elif method in ["dwt", "discrete wavelet transform"]:
        waves = _dwt_scg_delineator(scg_cleaned, rpeaks, sampling_rate=sampling_rate)

    else:
        raise ValueError("NeuroKit error: scg_delineate(): 'method' should be one of 'peak'," "'cwt' or 'dwt'.")
    ...
    instant_peaks = signal_formatpeaks(waves_noNA, desired_length=len(scg_cleaned))
    signals = instant_peaks

    if show is True:
        _scg_delineate_plot(
            scg_cleaned, rpeaks=rpeaks, signals=signals, signal_features_type=show_type, sampling_rate=sampling_rate
        )

    if check is True:
        waves = _scg_delineate_check(waves, rpeaks)
...
def _dwt_scg_delineator(scg, rpeaks, sampling_rate, analysis_sampling_rate=2000):
    """Delinate scg signal using discrete wavelet transforms.
    ...
    """
    scg = signal_resample(scg, sampling_rate=sampling_rate, desired_sampling_rate=analysis_sampling_rate)
    dwtmatr = _dwt_compute_multiscales(scg, 9)
    ...
    tpeaks, ppeaks = _dwt_delineate_tp_peaks(scg, rpeaks_resampled, dwtmatr, sampling_rate=analysis_sampling_rate)
    ...
    return dict(
        scg_T_Peaks=_dwt_resample_points(tpeaks, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_T_Onsets=_dwt_resample_points(tonsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_T_Offsets=_dwt_resample_points(toffsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_P_Peaks=_dwt_resample_points(ppeaks, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_P_Onsets=_dwt_resample_points(ponsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_P_Offsets=_dwt_resample_points(poffsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_R_Onsets=_dwt_resample_points(qrs_onsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
        scg_R_Offsets=_dwt_resample_points(qrs_offsets, analysis_sampling_rate, desired_sampling_rate=sampling_rate),
    )
...
def _dwt_delineate_tp_peaks(
    ...
    for rpeak_ in rpeaks:
        ...
        scg_local = scg[srch_idx_start:srch_idx_end]
        ...
        for idx_peak, idx_peak_nxt in zip(peaks[:-1], peaks[1:]):
            ...
            if correct_sign:
                ...
                # selected.
                score = scg_local[idx_zero] - (float(idx_zero) / sampling_rate - (rt_duration - 0.5 * qrs_width))
...
def _dwt_delineate_tp_onsets_offsets(
    ...
    for i in range(len(peaks)):  # pylint: disable=C0200
        ...
        # plt.plot(scg[srch_idx_start: srch_idx_end], '--', label='scg')
        # plt.show()
...
def _dwt_compute_multiscales(scg: np.ndarray, max_degree):
    ...
    dwtmatr = [arr[: len(scg)] for arr in dwtmatr]  # rescale transforms to the same length
...
# WAVELET METHOD (CWT)
# =============================================================================
def _scg_delineator_cwt(scg, rpeaks=None, sampling_rate=1000):

    # P-Peaks and T-Peaks
    tpeaks, ppeaks = _peaks_delineator(scg, rpeaks, sampling_rate=sampling_rate)

    # qrs onsets and offsets
    qrs_onsets, qrs_offsets = _onset_offset_delineator(scg, rpeaks, peak_type="rpeaks", sampling_rate=sampling_rate)

    # ppeaks onsets and offsets
    p_onsets, p_offsets = _onset_offset_delineator(scg, ppeaks, peak_type="ppeaks", sampling_rate=sampling_rate)

    # tpeaks onsets and offsets
    t_onsets, t_offsets = _onset_offset_delineator(scg, tpeaks, peak_type="tpeaks", sampling_rate=sampling_rate)

    # Return info dictionary
    return {
        "scg_P_Peaks": ppeaks,
        "scg_T_Peaks": tpeaks,
        "scg_R_Onsets": qrs_onsets,
        "scg_R_Offsets": qrs_offsets,
        "scg_P_Onsets": p_onsets,
        "scg_P_Offsets": p_offsets,
        "scg_T_Onsets": t_onsets,
        "scg_T_Offsets": t_offsets,
    }
...
def _onset_offset_delineator(scg, peaks, peak_type="rpeaks", sampling_rate=1000):
    # Try loading pywt
    try:
        import pywt
    except ImportError:
        raise ImportError(
            "NeuroKit error: scg_delineator(): the 'PyWavelets' module is required for this method to run. ",
            "Please install it first (`pip install PyWavelets`).",
        )
    # first derivative of the Gaissian signal
    scales = np.array([1, 2, 4, 8, 16])
    cwtmatr, __ = pywt.cwt(scg, scales, "gaus1", sampling_period=1.0 / sampling_rate)
...
def _peaks_delineator(scg, rpeaks, sampling_rate=1000):
    # Try loading pywt
    try:
        import pywt
    except ImportError:
    ...
    for i in range(len(rpeaks) - 1):
        ...
        significant_peaks_groups.append(_find_tppeaks(scg, significant_peaks_tp, sampling_rate=sampling_rate))
...
def _find_tppeaks(scg, keep_tp, sampling_rate=1000):
    # Try loading pywt
    try:
        import pywt
    except ImportError:
    ...
    for index_cur, index_next in zip(keep_tp[:-1], keep_tp[1:]):
        ...
        if correct_sign:
            ...
            index_max = np.argmax(scg[index_zero_cr - nb_idx : index_zero_cr + nb_idx]) + (index_zero_cr - nb_idx)
...
#                              PEAK METHOD
# =============================================================================
def _scg_delineator_peak(scg, rpeaks=None, sampling_rate=1000):

    # Initialize
    heartbeats = scg_segment(scg, rpeaks, sampling_rate)
    ...
    for i, rpeak in enumerate(rpeaks):
        ...
        # Peaks ------
        # Q wave
        Q_index, Q = _scg_delineator_peak_Q(rpeak, heartbeat, R)
        Q_list.append(Q_index)

        # P wave
        P_index, P = _scg_delineator_peak_P(rpeak, heartbeat, R, Q)
        P_list.append(P_index)

        # S wave
        S_index, S = _scg_delineator_peak_S(rpeak, heartbeat)
        S_list.append(S_index)

        # T wave
        T_index, T = _scg_delineator_peak_T(rpeak, heartbeat, R, S)
        T_list.append(T_index)

        # Onsets/Offsets ------
        P_onsets.append(_scg_delineator_peak_P_onset(rpeak, heartbeat, R, P))
        T_offsets.append(_scg_delineator_peak_T_offset(rpeak, heartbeat, R, T))
...
def _scg_delineator_peak_Q(rpeak, heartbeat, R):
...
def _scg_delineator_peak_P(rpeak, heartbeat, R, Q):
    if Q is None:
        return np.nan, None
...
def _scg_delineator_peak_S(rpeak, heartbeat):
...
def _scg_delineator_peak_T(rpeak, heartbeat, R, S):
    if S is None:
        return np.nan, None
...
def _scg_delineator_peak_P_onset(rpeak, heartbeat, R, P):
    if P is None:
        return np.nan
...
def _scg_delineator_peak_T_offset(rpeak, heartbeat, R, T):
    if T is None:
        return np.nan
...
def _scg_delineate_plot(scg_signal, rpeaks=None, signals=None, signal_features_type="all", sampling_rate=1000):
...
#                            signal_features_type='all', sampling_rate=1000)
    ...
    data = pd.DataFrame({"Signal": list(scg_signal)})
    ...
    for feature in data_cols:
        if signal_features_type == "peaks":
            ...
            if any(x in str(feature) for x in ["scg_R_Onsets", "scg_R_Offsets"]):
                df = data[feature]
                dfs.append(df)
        elif signal_features_type == "bounds_T":
            if any(x in str(feature) for x in ["scg_T_Onsets", "scg_T_Offsets"]):
                df = data[feature]
                dfs.append(df)
        elif signal_features_type == "bounds_P":
            if any(x in str(feature) for x in ["scg_P_Onsets", "scg_P_Offsets"]):
                df = data[feature]
                dfs.append(df)
...
def _scg_delineate_check(waves, rpeaks):
    ...
    df = pd.concat([df, pd.DataFrame({"scg_R_Peaks": rpeaks})], axis=1)
...
def _calculate_abs_z(df, columns):
    ...
    features by `scg_delineate()`"""
    for column in columns:
        df["Dist_R_" + column] = np.abs(standardize(df[column].sub(df["scg_R_Peaks"], axis=0)))
    return df
