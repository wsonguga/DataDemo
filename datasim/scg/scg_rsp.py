...
def scg_rsp(scg_rate, sampling_rate=1000, method="vangent2019"):
    ...
    if method in [
        "sarkar2015"
    ]:  # https://www.researchgate.net/publication/304221962_Extraction_of_respiration_signal_from_scg_for_respiratory_rate_estimation # noqa: E501
        rsp = signal_filter(scg_rate, sampling_rate, lowcut=0.1, highcut=0.7, order=6)
    elif method in ["charlton2016"]:  # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5390977/#__ffn_sectitle
        rsp = signal_filter(scg_rate, sampling_rate, lowcut=4 / 60, highcut=60 / 60, order=6)
    elif method in ["soni2019"]:  # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6329220/
        rsp = signal_filter(scg_rate, sampling_rate, highcut=0.5, order=6)
    elif method in [
        "vangent2019"
        ...
        rsp = signal_filter(scg_rate, sampling_rate, lowcut=0.1, highcut=0.4, order=2)
    else:
        raise ValueError(
            "NeuroKit error: scg_rsp(): 'method' should be "
